...
public class AnimalFarm {
	...
	AnimalFarm() {
		...
		int banana = 0;
		while (banana==0){
			String favAnimal = JOptionPane.showInputDialog("What is your favorite animal cow, duck, or dog. To quit out type: banana");
			if (favAnimal.equalsIgnoreCase("cow")) {
				playMoo();
			}
			else if (favAnimal.equalsIgnoreCase("duck")) {
				playQuack();
			}
			else if (favAnimal.equalsIgnoreCase("dog")) {
				playWoof();
			}
			else if (favAnimal.equalsIgnoreCase("banana")) {
				banana = banana+1;
			}
			else {
				JOptionPane.showMessageDialog(null, "You're an ugly swan");
			}
		}		}
		//for (int i = 0; i < 3; i++) {
